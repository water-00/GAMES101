...
void rst::rasterizer::draw(std::vector<Triangle *> &TriangleList) {
    ...
    {
        // 将一个三角形指针t解引用，并将其复制到一个新的三角形对象newtri中。这样可以保持原始三角形对象不变，从而避免对其进行修改
        Triangle newtri = *t;

        // 对每个三角形，记录三角形顶点经过MV变换后的坐标
        ...
        // 用了lambda表达式：[](auto& v) {return v.template head<3>();}
        // viewspace_pos就放了经过MV变换后的三个顶点的三维坐标
        std::transform(mm.begin(), mm.end(), viewspace_pos.begin(), [](auto& v) {
            return v.template head<3>();
        });

        // v记录经过MVP变换后三角形三个顶点的坐标
        Eigen::Vector4f v[] = {
                mvp * t->v[0],
                mvp * t->v[1],
                mvp * t->v[2]
        ...
        // 得到齐次坐标
        for (auto& vec : v) {
            vec.x()/=vec.w(); // w在main函数里实际上被规定为1.0
            vec.y()/=vec.w();
            vec.z()/=vec.w();
        }

        // 计算MV变换后的顶点的法向量，记录在n中
        ...
        //Viewport transformation 视口变换，得到顶点在屏幕上的坐标 即screen space
        for (auto& vert : v)
        {
...
void rst::rasterizer::rasterize_triangle(const Triangle& t, const std::array<Eigen::Vector3f, 3>& view_pos) 
{
    ...
    // 建立包围盒
    float trangle_x[3] = {t.v[0].x(), t.v[1].x(), t.v[2].x()};
    float trangle_y[3] = {t.v[0].y(), t.v[1].y(), t.v[2].y()};
    ...
    //      v[i].w() = 每个顶点视图空间深度, 也就是到摄像机的距离, 这是它和顶点的z坐标的区别
    //    * Z is interpolated view space depth for the current pixel
    //      Z = 当前像素用顶点坐标和像素坐标做重心插值后得到的视图空间深度, 即当前像素到摄像机的距离
    //    * zp is depth between zNear and zFar, used for z-buffer

    bool isInside[700][700]; // question: 700*700是哪来的？这里写死，难道以后每个作业都是700*700吗

    // 用depth_buf和frame_buf
    for (int i = std::floor(min_x); i <= max_x; i++) {
        for (int j = std::floor(min_y); j <= max_y; j++) {
            isInside[i][j] = insideTriangle(i + 0.5, j + 0.5, _v);
            if (isInside[i][j] == true) {
                // If pixel is inside the triangle, use the following code to get the interpolated z value.
